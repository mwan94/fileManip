/*
      ...
      v.1.2   updated on: Monday   02/04/2019
      ...
      Update V.1.2:
        > I added a code to delete any old report file before creating a new one.
...
int main(int argc, char *argv[]){
   ...
   printf("checking for old report files to delete!!!!\n\n");
   check1 = remove(addressDest);
   if (check1 == 0){printf("old %s file was deleted!\n\n", addressDest);}
   check2 = remove(argv[3]);
   if (check2 == 0){printf("old %s file was deleted!\n\n", argv[3]);}
...
//|----------------------------------- and send commands to the terminal to be executed ----------------------------------------|
      ...
      printf("Program has finished executing.... opening the report file in gedit\n\n\n");
      snprintf(command1, sizeof(command1), "gedit %s",addressDest);
      system(command1);
